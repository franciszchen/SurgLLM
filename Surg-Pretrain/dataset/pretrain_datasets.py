...
def adjust_masks(encoder_mask, foreground_masks):

    
    encoder_mask_count = sum(encoder_mask)

    combined_mask = np.logical_or(encoder_mask,foreground_masks).astype(int)

    combined_mask_count = sum(combined_mask)

    diff = int(combined_mask_count - encoder_mask_count)

    if diff>0:
        indices = np.where((combined_mask == 1) & (foreground_masks == 0))
        
        del_indices = np.random.choice(indices[0],diff,replace=False)
...
class HybridVideoMAE(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.mask_tube_len = mask_tube_len
    ...
    def __getitem__(self, index):
        try:
            ...
            self.skip_length = self.orig_skip_length #64
            self.new_step = self.orig_new_step #4
            
            if total_frame < 0: #case of video
                ...
                frame_id_list = self.get_frame_id_list(duration,
                                                       segment_indices,
                                                       skip_offsets) 
                #e.g. [28393, 28397, 28401, 28405, 28409, 28413, 28417, 28421, 28425, 28429, 28433, 28437, 28441, 28445, 28449, 28453] skip size = 4
                #手动加入mask
                if self.mask_path:
                    json_name = "VID"+ video_name.split("/")[-1][3:5]
                    #手动读取mask
                    mask = self.mask_json[json_name]
                    pre_foreground_masks = np.array([mask[str(i)] for i in frame_id_list][::self.tubelet_size]) #e.g. [::2] 2是tubelet size
                    new_len = pre_foreground_masks.shape[0]//(self.mask_tube_len//self.tubelet_size)
                    new_array = np.zeros((new_len, pre_foreground_masks.shape[1]))
                    for i in range(new_len):
                        new_array[i] = pre_foreground_masks[i * (self.mask_tube_len//self.tubelet_size)]  # 每组的第一个元素
                    foreground_masks = np.repeat(new_array, (self.mask_tube_len//self.tubelet_size), axis=0)
                ...
                segment_indices, skip_offsets = self._sample_train_indices(
                    total_frame) #得到随机的segment_indices以及长度为16//2的 zero list的skip offsets
        ...
        if self.num_sample > 1:
            ...
            for i in range(self.num_sample):
                ...
                if self.mask_path:
                    lens,_ = encoder_mask.shape
                    combined_mask = []
                    for j in range(lens):
                        encoder_mask_one_dim = adjust_masks(encoder_mask[j],foreground_masks[j])
                        combined_mask.append(encoder_mask_one_dim)

                    encoder_mask = np.array(combined_mask)

                    decoder_mask = 1-encoder_mask #opposite to the encoder mask
    ...
    def _sample_train_indices(self, num_frames):
        ...
        if self.temporal_jitter:
            ...
            skip_offsets = np.zeros(
                self.skip_length // self.new_step, dtype=int) #[0]*16
    ...
    def get_frame_id_list(self, duration, indices, skip_offsets):
        ...
        for seg_ind in indices:
            offset = int(seg_ind)
            for i, _ in enumerate(range(0, self.skip_length, self.new_step)): #一个包含16个frame的list,每两个一取
