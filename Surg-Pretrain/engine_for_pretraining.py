...
def train_one_epoch(model: torch.nn.Module,
    ...
    for step, batch in enumerate(
        ...
        bool_masked_pos = bool_masked_pos.to(
            device, non_blocking=True).flatten(1).to(torch.bool) #4,2048; 2048 = 8*256
        ...
        with torch.no_grad():
            ...
            std = torch.as_tensor(IMAGENET_DEFAULT_STD).to(device)[None, :,
                                                                   None, None,
                                                                   None]
            unnorm_images = images * std + mean  # in [0, 1] [4,3,16,224,224]

            if normlize_target:
                images_squeeze = rearrange(
                    unnorm_images,
                    'b c (t p0) (h p1) (w p2) -> b (t h w) (p0 p1 p2) c', #b=4,c=3,t=8,p0=2,h=16,p1=14,w=16,p2=14 - > 4 (8*16*16) (14*14*2) *3
                    p0=tubelet_size,
                    p1=patch_size,
                    p2=patch_size)
            ...
            B, N, C = images_patch.shape #[4, 2048, 1176] if num_frame=16
