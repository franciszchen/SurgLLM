...
def auto_load_model(args,
    ...
    if loss_scaler is not None:
        ...
        if args.resume:
            ...
            #     current_state_dict = model_without_ddp.state_dict()
            #     filtered_dict = {k: v for k, v in pretrained_ckpt.items() if k in current_state_dict and current_state_dict[k].size() == v.size()}
            #     current_state_dict.update(filtered_dict)
