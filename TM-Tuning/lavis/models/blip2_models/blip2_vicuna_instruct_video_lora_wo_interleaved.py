...
@registry.register_model("blip2_vicuna_instruct_video_lora_wo_interleave")
class Blip2VicunaInstructVideoLoRA_WO_INTERLEAVE(Blip2Base):
    """
    BLIP2 Vicuna model.
    ...
    def warp_video_embedding_with_words(self,query_output_list,query_tokens,cur_device):
        ...
        for i in range(len(query_output_list)):
            if i == 0:
                inputs_embeds = None
                attention_mask = None
                video_text = "Here is a list of videos in order. Each video is 4 seconds. ".format(begin=i*4,end=i*4+4) #text input
