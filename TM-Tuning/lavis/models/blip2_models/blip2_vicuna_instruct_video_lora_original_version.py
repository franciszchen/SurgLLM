...
config.model.text_encoder["config"] = "/ssd/lxj/project/videollm/multi_modality/configs/config_bert_large.json"
config.model.vision_encoder["num_frames"] = 4
config.model.text_encoder["pretrained"] = "/ssd/lxj/checkpoint/bert-large-uncased"



@registry.register_model("blip2_vicuna_instruct_video_lora_old")
class Blip2VicunaInstructVideoLoRA(Blip2Base):
    """
    BLIP2 Vicuna model.
    ...
    def __init__(
        ...
        freeze_vit=True,
        num_query_token=32,
        llm_model="/ssd/lxj/checkpoint/vicuna-7b-v1.1",
        prompt="",
        max_txt_len=128,
    ...
    def warp_video_embedding_with_words(self,query_output_list,query_tokens,cur_device):
        ...
        for i in range(len(query_output_list)):
            if i == 0:
                inputs_embeds = None
                attention_mask = None
                video_text = "Here is a list of videos in order. From time {begin}s to {end}s : ".format(begin=i*4,end=i*4+4) #text input
                ...
                inputs_embeds_ = self.llm_model.get_input_embeddings()(torch.tensor(input_id['input_ids'][1:])) #text input embedding
                ...
                video_text = "From time {begin}s to {end}s : ".format(begin=i*4,end=i*4+4) 
                ...
                inputs_embeds_ = self.llm_model.get_input_embeddings()(torch.tensor(input_id['input_ids'][1:]))
    ...
    def forward(self, samples):
        ...
        #     cur_device = image.device
        #     bs = image.size(0)

        #     query_tokens = self.query_tokens.expand(image_embeds.shape[0], -1, -1).to(cur_device)
        #     if self.qformer_text_input:
        #         text_Qformer = self.tokenizer(
        #             samples["text_input"],
        #             padding='longest',
        ...
        # for query_output in query_output_list:
        ...
        empty_targets = (
            torch.ones(atts_llm.size(), dtype=torch.long).to(cur_device).fill_(-100)
        )
    ...
    def generate(
        ...
        if self.video_cfg:
            ...
            for path in video_paths:
                video = cv2.VideoCapture(path)
                frames = [x for x in self._frame_from_video(video)]
                vid_tube = self.frames2tensor(frames)
                if vid_tubes is None:
                    vid_tubes = vid_tube
                else:
                    vid_tubes = torch.cat((vid_tubes,vid_tube),dim=0)
            ...
            #     ).to(cur_device)
            #     query_atts = torch.ones(query_tokens.size()[:-1], dtype=torch.long).to(cur_device)
            #     Qformer_atts = torch.cat([query_atts, text_Qformer.attention_mask], dim=1).to(cur_device)
