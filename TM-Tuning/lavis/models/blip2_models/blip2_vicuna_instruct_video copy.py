...
model_pth = '/mnt/xingjian_luo/project/videollm/video_encoder/mp_rank_00_model_states.pt'
config['pretrained_path'] = model_pth
config['origin_num_frames'] = 4
config.model.text_encoder["config"] = "/mnt/xingjian_luo/project/videollm/multi_modality/configs/config_bert_large.json"
...
@registry.register_model("blip2_vicuna_instruct_old")
class Blip2VicunaInstructVideo(Blip2Base):
    ...
    def __init__(
        ...
        freeze_vit=True,
        num_query_token=32,
        llm_model="/mnt/xingjian_luo/checkpoint/vicuna-7b-v1.1",
        prompt="",
        max_txt_len=128,
        ...
        if video_cfg:
            ...
            num_query_token, self.video_cfg.embed_dim
        )
        # self.Qformer, self.query_tokens = self.init_Qformer(
    ...
    def get_video_tensor(self,video_path):
    ...
    def forward(self, samples):
        ...
        #先提取到视频片段
        if self.video_cfg:
            video = samples["video"] #should be a list of videos
            with self.maybe_autocast():
                #video_embeds : [1, 1025, 1408], pooled : [1, 768] pooled是经过了一个attention池化层，把query做了一个平均，用一个平均的query去做attention，得到一个一维的维度
                video_embeds,pooled_vision_embeds = self.get_video_tensor(video)
            video_atts = torch.ones(video_embeds.size()[:-1], dtype=torch.long).to(image.device)
            bs = video.size(0) #batch size of videos
        ...
        if self.video_cfg:
            ...
            if self.qformer_text_input:
                query_output = self.Qformer.bert(
                    ...
                    attention_mask=Qformer_atts,
                    query_embeds=query_tokens,
                    encoder_hidden_states=video_atts,
                    encoder_attention_mask=image_atts,
                    return_dict=True,
                )
            else:
