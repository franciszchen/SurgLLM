...
class MultiModalDatasetBuilder(BaseDatasetBuilder):
        ...
        self.processors = {
            split: {
                modality: self._build_proc_from_cfg(
                    self.config.get(f"{'vis' if 'video' in modality else modality}_processor").get(split)
                )
    ...
    def _get_dataset_args(self, info, is_train):
        ...
        for modality in self.data_type:
            proc_name = f"{'vis' if 'video' in modality else modality}_processor"
            ...
            dataset_args[f"{'vis' if 'video' in modality  else modality}_root"] = mm_path
        
        dataset_args['text_processor'] = self.text_processors["train" if is_train else "eval"]
        dataset_args["ann_paths"] = [info.storage]
        #dataset_args['modalities'] = self.data_type
        
        # Conform to base
        for key in ['vis_processor', 'vis_root']:
