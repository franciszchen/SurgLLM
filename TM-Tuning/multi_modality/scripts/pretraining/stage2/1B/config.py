...
# ========================= data ==========================
train_file = available_corpus["cholect50_short_train"]

test_file = dict(msrvtt_1k_test=available_corpus["cholect50_short_test"])

test_types = ["msrvtt_1k_test"]
num_workers = 8

best_key = ["cholect50_short_test","cholect50_short_test"]
...
model = dict(
    ...
    vision_encoder=dict(
        ...
        pretrained='/mnt/xingjian_luo/project/VideoMAEv2/vit_g_hybrid_pt_foreground_mask_0809_frame_16_mask_tube_length_2_4/checkpoint-599.pth',
...
scheduler = dict(sched="cosine", epochs=30, min_lr_multi=0.01, warmup_epochs=1)
...
save_latest = True
auto_resume = False
jump_evaluate = True #jump evluation
pretrained_path = "/mnt/xingjian_luo/checkpoint/models--OpenGVLab--InternVideo2-Stage2_1B-224p-f4/InternVideo2-stage2_1b-224p-f4.pt"  # path to pretrained model weights, for resume only?
save_ckpt_iter = None
